...
slang_word_test = {'dpt':'dapat','krn':'karena','utk':'untuk','tp':'tapi','sy':'saya',
...
for conten in testing['Content']:
    txt = replace_semua(conten.lower(), slang_word_test)
    slang_test.append(txt)

testing['slang_test'] = slang_test
...
for conten in slang_test:
    output   = stemmer.stem(conten)
    st_test.append(output)

testing['stemmed_test'] = st_test

#%%

#Pisahkan kata/tokenize data test

PK= [nltk.word_tokenize(PisahKata_test) for PisahKata_test in st_test]
...
stopword_test = ['ada','adalah','adanya','adapun','agak','agaknya','agar','akan','akankah',
...
for conten in PK:
    kata= filter(lambda x: x not in stopword_test, conten)
    sw_test.append(kata)

testing['stopword_test'] = sw_test


#%%

#Ubah list ke str
string_test = map(' '.join, sw_test)
